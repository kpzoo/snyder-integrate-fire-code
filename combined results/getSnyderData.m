...
function [betaSny, gammaSny, meanSny, mseSny, L, U, mseRef, betaRef] = getSnyderData(locFol, thisDir, numIPP, lenb, leng)
...
for i = 1:numIPP
    IPPstruc = load(ippFile(i).name, 'mseEst', 'meanEst', 'betaSet', 'gammaSet');
    for j = 1:leng
        betaSny(i, :, j) =  IPPstruc.betaSet(j, :);
        meanSny(i, :, j) =  IPPstruc.meanEst(j, :);
        mseSny(i, :, j) =  IPPstruc.mseEst(j, :);
        gammaSny(i, :, j) =  IPPstruc.gammaSet(j, :);
    end
end
cd(thisDir);
